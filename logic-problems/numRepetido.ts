...
let numRepetido = [1, 2, 3, 4, 5, 1, 2, 1, 6, 7, 8, 1];
...
function numeroMasRepetido(numeros: number[]): number {
  let maxRepeticiones = 0;
  let numeroResultado = numeros[0];

  for (let i = 0; i < numeros.length; i++) {
    let contador = 1; // se reinicia por cada número
